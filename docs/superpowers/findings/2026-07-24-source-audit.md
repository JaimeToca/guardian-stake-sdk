...
**Total findings by severity (across all five packages, systemic finding counted once with 4 sub-instances):**

| Severity | Count | Breakdown |
|---|---|---|
| Critical | 0 | — |
| High | 1 systemic (4 instances) | bsc, cardano, tron, solana — each package's own `compile()` |
| Medium | 9 | sdk 1, bsc 2 (M-2, M-3; M-1 counted separately below), cardano 2, tron 2, solana 4 |
| Low | 15 | sdk 3, bsc 4, cardano 3, tron 3, solana 3 (approx; see table for exact per-package tally) |
| Informational | 24 | sdk 5, bsc 3, cardano 5, tron 7, solana 3 (see table) |

Exact per-package counts as stated by each auditor: sdk (0/0/1/3/4... wait, see below), bsc (0/1/3/4/3), cardano (0/2/2/3/5), tron (0/1/2/3/7), solana (0/1/4/3/3). Note: bsc's "1 High" and cardano's "2 High" both refer to the same underlying `compile()`-signature-verification defect described independently per package — this document treats that class as **one systemic finding (SEC-SIGN-1)** with a sub-row per package, so it is *not* double-counted in the executive total above. The Medium/Low/Informational counts in the table in Section 3 are the literal per-finding counts from each report.
