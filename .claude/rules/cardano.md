...
**`@cardano-sdk/*` are exact-pinned bundled dependencies** — they live in `dependencies` (not `peerDependencies`) and are pinned to exact releases (`0.46.12`, `0.4.5`, `0.17.1`) because the family has no stability guarantees between minors and CBOR serialisation is sensitive to the exact release. Bundling (not peering) is deliberate: these are internal plumbing that never crosses the SDK's public API — consumers exchange hex strings, not `@cardano-sdk` objects — so bundling gives them the exact tested versions automatically with no manual install. Contrast `viem` in bsc, which *is* a peer because a viem `PrivateKeyAccount` crosses the boundary via `SigningWithAccount` and must be a single shared copy. `@guardian-sdk/sdk` remains the only Cardano peer. If you change a version or move these between `dependencies` and `peerDependencies`, run `pnpm install` immediately — forgetting causes CI to fail with `ERR_PNPM_OUTDATED_LOCKFILE`.
