import { buildUrl, performOrError } from "../utils/rpc-utils";
...
export class RpcClient implements RpcClientContract {
  ...
  async getValidators(): Promise<SmartChainValidator[]> {
    const requestUrl = buildUrl(RpcClient.BASE_MAINNET_URL, "/validator/all", {
    ...
    const validatorResponse = await performOrError<SmartChainValidatorsResponse>(request);
